...
export default function ModulePage() {
  ...
  const [view, setView] = useState<ModuleView>("menu");
  const [quizState, setQuizState] = useState<QuizState>({ index: 0, answers: [], done: false, selected: null });
  const [flashState, setFlashState] = useState<FlashState>({ index: 0, flipped: false });
  const [stepState, setStepState] = useState<StepState>({ index: 0 });
  const [scenarioState, setScenarioState] = useState<ScenarioState>({ scenarioIndex: 0, decisionIndex: 0, selected: null, history: [], done: false });
  ...
  const navigate = useCallback((targetView: ModuleView, setupFn?: () => void) => {
    tr(() => { setupFn && setupFn(); setView(targetView); });
  }, [tr]);
  ...
  if (view === "menu") {
    ...
    return (<div style={S.app}><div style={S.hdr}><button style={S.back} onClick={()=>router.push("/")}>←</button><div><div style={{fontSize:15,fontWeight:700,display:"flex",alignItems:"center",gap:6}}><topic.icon size={18} strokeWidth={1.75} color={topic.color} />{topic.title}</div><div style={{fontSize:11,color:"#666"}}>{topic.subtitle}</div></div></div>
  ...
  if (view === "steps" && topic.steps) {
    ...
    return (<div style={S.app}><div style={S.hdr}><button style={S.back} onClick={()=>navigate("menu")}>←</button><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600}}>Step {stepState.index+1}/{topic.steps.length}</div><Prog c={stepState.index+1} t={topic.steps.length}/></div></div>
      <div ref={ref} style={bodyStyle}><div style={{padding:"20px 0 10px"}}>
        ...
        {currentStep.video && <a href={`https://youtu.be/${currentStep.video}`} target="_blank" rel="noopener noreferrer" style={{display:"block",margin:"0 0 16px",borderRadius:11,overflow:"hidden",border:"1px solid #ffffff14",position:"relative",textDecoration:"none",aspectRatio:"16/9",background:"#111"}}><img src={`https://img.youtube.com/vi/${currentStep.video}/hqdefault.jpg`} alt={currentStep.title + " video"} style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.85}}/><div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:48,height:48,borderRadius:24,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:0,height:0,borderTop:"10px solid transparent",borderBottom:"10px solid transparent",borderLeft:"18px solid #fff",marginLeft:3}}/></div><div style={{position:"absolute",bottom:0,left:0,right:0,padding:"8px 12px",background:"linear-gradient(transparent,rgba(0,0,0,0.8))"}}><div style={{fontSize:10,color:"#fff",fontWeight:600}}>Watch Procedure Video</div></div></a>}
  ...
  if (view === "quiz" && topic.quiz) {
    ...
    return (<div style={S.app}><div style={S.hdr}><button style={S.back} onClick={()=>navigate("menu")}>←</button><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600}}>Q {quizState.index+1}/{topic.quiz.length}</div><Prog c={quizState.index+1} t={topic.quiz.length}/></div></div>
      <div ref={ref} style={bodyStyle}><div style={{padding:"20px 0 14px"}}><h3 style={{fontSize:16,fontWeight:600,lineHeight:1.5,margin:0}}>{currentQuestion.q}</h3></div>
        ...
        {hasAnswered&&<div style={{marginTop:12}}><button style={S.btn(topic.color,true)} onClick={()=>{const updatedAnswers=[...quizState.answers,quizState.selected as number];quizState.index<topic.quiz!.length-1?tr(()=>setQuizState({index:quizState.index+1,answers:updatedAnswers,done:false,selected:null})):tr(()=>setQuizState({...quizState,answers:updatedAnswers,done:true}))}}>{quizState.index<topic.quiz.length-1?"Next":"Results"}</button></div>}
      </div><Bar active="train"/></div>);
  }

  // FLASHCARDS with spaced repetition
  if (view === "flashcards" && topic.flashcards) {
