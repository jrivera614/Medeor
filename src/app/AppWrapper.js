      ...
      {emailCapture.show && cookieConsent !== null && (
        <div style={{position:"fixed",bottom:68,left:"50%",transform:"translateX(-50%)",width:"calc(100% - 32px)",maxWidth:448,background:"#1a1a24",border:"1px solid #8b5cf625",borderRadius:14,padding:"14px 16px",zIndex:45,boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <input type="email" placeholder="your@email.com" value={emailCapture.email} onChange={e=>setEmailCapture(p=>({...p,email:e.target.value,error:null}))} style={{...S.input,flex:1,padding:"9px 12px",fontSize:12}}/>
            <button disabled={emailCapture.submitting} onClick={async()=>{
              const email = emailCapture.email.trim();
              if(!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){setEmailCapture(p=>({...p,error:"Please enter a valid email address"}));return;}
              setEmailCapture(p=>({...p,submitting:true,error:null}));
              try{
                const res = await fetch("https://formspree.io/f/xkoqyklw",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,type:"subscribe"})});
            ...
            }} style={{padding:"9px 14px",background:emailCapture.submitting?"#555":"#8b5cf6",border:"none",borderRadius:9,color:"#fff",fontSize:12,fontWeight:600,cursor:emailCapture.submitting?"default":"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{emailCapture.submitting?"...":"Subscribe"}</button>
          </div>
